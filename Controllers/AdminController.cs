...
public ActionResult UrunEkle(URUNLER u, HttpPostedFileBase ResimDosyasi)
{
    ...
    return RedirectToAction("UrunListesi");
        ...
        [HttpPost]
        public ActionResult Subeler(SUBELER k)
        {
            SehirListesiGetir();

            if (!ModelState.IsValid)// şartlar uymuyorsa bu sayfada kal
            {
                return View();
            }

            SUBELER yeni = new SUBELER();
        ...
        public void SehirListesiGetir()
        {
            var iller = new List<string>() { "Adana", "Adıyaman", "Afyon", "Ağrı", "Amasya", "Ankara",
                ...
                "Erzurum", "Eskişehir", "Gaziantep", "Giresun", "Gümüşhane", "Hakkari", "Hatay", "Isparta", "İçel (Mersin)",
                "İstanbul", "İzmir", "Kars", "Kastamonu", "Kayseri", "Kırklareli", "Kırşehir", "Kocaeli", "Konya", "Kütahya",
